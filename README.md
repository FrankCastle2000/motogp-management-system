...
- 多赛季车手积分榜，支持管理员创建、切换赛季及受频率限制的官方同步；只有确认当季车手与车队名单后才开放积分录入
- 多赛季网格化赛程日历、新赛季官方分站自动建立、北京时间日程同步，以及已完赛分站成绩一键批量同步
- 分站国家中文名采用跨赛季映射；官网缺少内置译名时标记待确认，并保存管理员修正结果
- 管理员经赛季年份二次确认后，可事务化删除整季名单、积分、赛程、日程及比赛成绩
- 按赛季独立维护车手、车队、所属关系及车辆信息，支持搜索、排序、增删改与并发修改保护
- 车手编号按赛季保存并保证同一赛季内唯一，不同赛季可由不同车手复用同一号码
- 车手中文名按统一身份跨赛季复用；管理员确认后的标准译名不会被官网同步覆盖
- 管理员按赛季从 MotoGP 官方接口同步车队、制造商与车手详细资料
...
![MotoGP 多赛季车手积分榜](docs/screenshots/standings.png)
...
系统不允许提前创建未来赛季。新赛季建立后，应先在“车队信息”和“车手信息”页面录入或同步当季名单，再由管理员点击“确认名单完成”；名单确认前，手工新增积分和刷新官方积分都会被禁用。名单确认后如再次修改车手或车队，系统会自动撤销确认，需要重新核对并确认。
